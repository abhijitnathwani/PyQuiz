...
class Application(tk.Frame):
    ...
    def __init__(self, master=None):
        ...
        """
        tkinter.messagebox.showinfo('Welcome!', 'Welcome to PyQuiz!\nA quiz built in Python to test your general '
                                                'knowledge.')
        tk.Frame.__init__(self, master)
        tk.Frame.focus_force(self)
        self.grid()
    ...
    def about(self):
        ...
        """
        tkinter.messagebox.showinfo("About PyQuiz!",
                                    "Welcome to PyQuiz! v0.3\n PyQuiz is developed to explore Tkinter and then "
                                    "started off as a "
                                    "simple application.\nPyQuiz! is maintained at "
                                    "\nhttps://github.com/abhijitnathwani/PyQuiz/ "
                                    "\n\nYour contributions and suggestions are welcome. Feel free to fork and pull "
                                    "changes to "
                                    "PyQuiz! The application is open-source and is open for development.\n\nPyQuiz is "
                                    "developed "
                                    "and "
                                    "maintained by Abhijit Nathwani. For suggestions and changes, feel free to drop "
                                    "an email:\n "
    ...
    def load_question(self, top):
        ...
        randomindex = random.randint(0,
                                     len(self.questions["results"]) - 1)  # generate random index to be picked from list
        ...
        self.optionA.set(self.answers.pop(random.randrange(len(
            self.answers))))  # randomly set the option label from the answers list and then remove from that list to
        #  avoid repetition
    ...
    def createWidgets(self, top):
        ...
        self.radioButtonA = tk.Radiobutton(self, anchor='w',
                                           ...
                                           value='A',
                                           command=lambda: self.set_ans(
                                               1))  # the radio button call 'set_ans()' with the number to set the
        # 'selected_answer' variable
